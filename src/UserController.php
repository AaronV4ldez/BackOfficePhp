...
class UserController extends BaseController
{
    ...
    public function __construct(
        ...
        $this->gateway = new UserGateway($this->getDatabase());
        $this->user_id = $user_id;
    }

    public function doCreate()
    {
        $errors = $this->getValidationErrors($this->params);
        if (!empty($errors)) {
            $this->respondUnprocessableEntity($errors);
            die;
        }
        $id = $this->gateway->createUser(
            $this->params["name"],
            $this->params["email"],
            $this->params["password"],
            $this->params["phone"],
            $this->params["usertype"],
            1 // if user created from this controller, we assume account verified (internal user)
        );
